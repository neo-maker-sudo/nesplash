...
const a = document.getElementsByClassName("travel-a")[2]
...
class Travel {
    ...
    display_travel(results){
        ...
        for(var i=0;i<results.length;i++){
            ...
            div.classList.add("travel-section-2-contributorDiv");
            img.classList.add("sec_2_contributorImgfile");
            p.classList.add("travel-section-2-contributorp");
            link_div.classList.add("travel-section-2-contributor-linkDiv")
            inside_a.classList.add("travel-section-2-contributor-inside-a");
            outside_a.classList.add("travel-section-2-contributor-outside-a")
    ...
    fetchData(page){
        ...
        const url = `http://localhost:5000/travel/api/contributor?page=${page}`;
        post_flag = true;
        fetch(url)
        .then( async (response)=>{
